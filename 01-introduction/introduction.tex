...
Once full decoupling is achieved, the corresponding free-streaming energy density, pressure, and number densities can be written as\index{free-streaming!energy density}\index{free-streaming!pressure}\index{free-streaming!number density}\index{pressure}\index{energy density}
\begin{align}
...
\end{align} 
which are different from the thermal equilibrium \req{energy_density}, \req{Pressure_density}, and \req{number_density}, by replacing the mass by a time dependant effective mass $m\,T_\mathrm{fs}(t)/T_\mathrm{F}$ in the exponential.
...
\end{align}
The square bracket has to vanish. Solving for $\dot T=dT/dt$ and taking the integral, the relation between time and temperature in the primordial Universe can be written as\index{Universe!time-temperature relation}
...
In~\rf{Fig:Overview} the black line presents the relation between time $t$\,[s] (ordinate, increasing scale) and temperature $T$\,[MeV](abscissa, decreasing scale)  during the first hour of the evolution of the Universe, reaching down to the temperature $T=10$\,keV. Vertical and horizontal lines indicate some characteristic events related to the Universe particle inventory, as marked. 
