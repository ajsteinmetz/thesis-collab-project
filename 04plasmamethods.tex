...
%\begin{multline}
\begin{align}
    (p \cdot \partial)\left(f_\mathrm{eq}+ \delta f(x,p)\right) +  q \left(F_{\mathrm{eq}}^{\mu\nu} +\delta F^{\mu\nu}\right)p_\nu &\frac{\partial (f_\mathrm{eq}+\delta f(x,p))}{\partial p^\mu} 
    = \kappa (p\cdot u)\left(f_\mathrm{eq} \frac{\delta n(x)}{{n_\mathrm{eq}(x)}} - \delta f(x,p)\right)\,.
%\end{multline}
\end{align}
...
\begin{equation}
    |\mathbf{k}| = \pm  i m_D \,.
\end{equation}
Fourier transforming using the positive root in \req{eq:phi} gives the Debye-H{\"u}ckel screening of a stationary charge within the plasma \cite{Debye_1923a,Debye_1923b}
\begin{equation}
...
\end{equation}
and their associated permeability $K_\parallel$ and $K_\perp$. These quantities are useful for studying the attenuation of electromagnetic fields by looking at light absorption.
