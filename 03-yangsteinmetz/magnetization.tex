...
\label{sec:abundance}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\noindent As the universe cooled below temperature $T\!=\!m_{e}$ (the electron mass), the thermal electron and positron\index{electron-positron plasma} comoving density depleted by over eight orders of magnitude. At $T_\mathrm{split}=20.3\keV$, the charged lepton asymmetry (mirrored by baryon asymmetry and enforced by charge neutrality) became evident as the surviving excess electrons persisted while positrons vanished entirely from the particle inventory of the universe due to annihilation.
...
\end{align}
where $n_{p}$ and $n_B$ is the number density of protons and baryons\index{baryon density} respectively. The parameter $s_{\gamma,e}$ is the entropy density\index{entropy density} which is primarily dominated by photons and electron(positrons) in this era. Due to the adiabatic expansion of the universe, the comoving entropy density is a conserved making the ratio
\begin{align}
