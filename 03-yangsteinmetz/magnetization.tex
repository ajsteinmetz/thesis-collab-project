...
The grand partition function\index{partition function} for the relativistic Fermi-Dirac\index{Fermi-Dirac distribution} distribution is given by the standard definition~\cite{Elze:1980er}
\begin{align}
...
\label{sec:eulermac}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\noindent Before we proceed with the Boltzmann distribution\index{Boltzmann!distribution} approximation which makes up the bulk of our analysis, we will comment on the full Fermi-Dirac\index{Fermi!distribution} distribution analysis. The Euler-Maclaurin formula~\cite{abramowitz1988handbook} is used to convert the summation over Landau levels $n$ into an integration given by
\begin{multline}
...
Summation over the auxillary function $W_{\nu}$ can be replaced via Euler-Maclaurin integration~\req{eulermaclaurin} as
\begin{align}
...
\label{sec:chem}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\noindent Considering the temperature after neutrino freeze-out, the charge neutrality\index{charge neutrality} condition can be written as
\begin{align}
...
\end{align}
where $n_{p}$ and $n_B$ is the number density of protons and baryons\index{baryon!entropy ratio} respectively. The parameter $s_{\gamma,e}$ is the entropy density which is primarily dominated by photons and electron(positrons) in this era. Due to the adiabatic expansion of the universe, the comoving entropy density is a conserved making the ratio
\begin{align}
...
\end{align}
For the simplest mediums without ferromagnetic or hysteresis considerations, the relationship can be parameterized by the susceptibility\index{magnetic!susceptibility} $\chi$ of the medium as
\begin{align}
...
We note that the expression for the magnetization simplifies significantly for $g\!=\!2$ which is the `natural' gyro-magnetic factor~\cite{Evans:2022fsu,Rafelski:2022bsv}\index{g-factor} for Dirac particles. For illustration, the $g\!=\!2$ magnetization from \req{defmagetization} is then
\begin{align}
 \label{g2magplus}
