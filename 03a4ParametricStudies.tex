...
\end{equation}
If the particle  type being considered in both directions of a reaction that the corresponding term for both directions  must  be added. We emphasize that the key difference here is there is no minus sign; here we are counting reactions, not net particle number change.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{natural constants!variation}
...
\end{align}
where $t_0$ denotes the present day values, we have
%$B=n_B/n_\gamma= 0.606\times10^{-9}$ \cite{ParticleDataGroup:2016lqr}
$\eta_\gamma\equiv n_B/n_\gamma= 0.614\pm0.002\times10^{-9}$ 
(CMB)~\cite{ParticleDataGroup:2022pth} \index{CMB} from today's observation. The entropy per particle for a massless boson at zero chemical potential\index{chemical potential} is $(s/n)_{\mathrm{boson}}\approx 3.602$.
...
The large lepton asymmetry\index{lepton!asymmetry} from cosmic neutrino can also affect the neutron lifespan in cosmic plasma which is one of the important parameter controlling BBN element abundances. 
...
\frac{dn_\nu}{dt}+3Hn_\nu=R_{e\overline{e}\to\nu\overline{\nu}}(T_{\gamma,e^\pm})-R_{\nu\overline{\nu}\to e\overline{e}}(T_\nu),
\end{align}
where $n_\nu$ is the number density of neutrinos and $H$ is the Hubble parameter\index{Hubble!parameter}. The parameter $T_{\gamma,e^\pm}$ is the equilibrium temperature between photons and $e^\pm$ and $T_\nu$ is the temperature for free-streaming\index{free-streaming} neutrinos: 
\begin{align}
