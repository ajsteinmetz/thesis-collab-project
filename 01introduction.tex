...
f_\mathrm{fs}(t,E)=\frac{1}{\Upsilon^{-1}\exp{\left[\sqrt{\frac{E^2-m^2}{T_\mathrm{fs}^2}+\frac{m^2}{T^2_\mathrm{F}}}-\frac{\mu}{T_\mathrm{F}}\right]+1}},\quad T_\mathrm{fs}(t)=\frac{T_\mathrm{F}a(t_F)}{a(t)},
\end{equation}
where $t_F$ is the time at which kinetic freeze-out occurs and the free-streaming effective temperature $T_\mathrm{fs}$ is obtained by redshifting the temperature at kinetic freeze-out. If a massive particle (e.g. dark matter) freeze-out from cosmic plasma in the nonrelativistic regime, $m\gg T_\mathrm{F}$. We can use the
